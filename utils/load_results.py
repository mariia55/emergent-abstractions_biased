...
def load_accuracies(
    ...
    for path_idx, path in enumerate(all_paths):
        ...
        for run in range(n_runs):
            ...
            _, val_acc = zip(*lists)
            if (
                len(val_acc) > n_epochs // val_steps
            ):  # old: we had some runs where we set val freq to 5 instead of 10
                val_acc = val_acc[::2]
            ...
            if zero_shot:
                for cond in ["specific", "generic"]:
                    ...
                    if context_unaware:
                        ...
                        if cond == "specific":
                            cu_zs_specific_test_accs.append(
                                cu_zs_data["final_test_acc"]
                            )
                        else:
            ...
            if context_unaware:
                cu_data = pickle.load(
                    open(context_unaware_path + "loss_and_metrics.pkl", "rb")
                )
...
def load_entropies(all_paths, n_runs=5, context_unaware=False, length_cost=0.0):
    ...
    result_dict = {
        ...
        "consistency": [],
        "NMI_hierarchical": [],
        "effectiveness_hierarchical": [],
        "consistency_hierarchical": [],
        "NMI_context_dep": [],
        "effectiveness_context_dep": [],
        "consistency_context_dep": [],
        "NMI_concept_x_context": [],
        "effectiveness_concept_x_context": [],
    ...
    for path_idx, path in enumerate(all_paths):
        ...
        (
            NMIs_hierarchical,
            effectiveness_scores_hierarchical,
            consistency_scores_hierarchical,
        ) = ([], [], [])
        (
            NMIs_context_dep,
            effectiveness_scores_context_dep,
            consistency_scores_context_dep,
        ) = ([], [], [])
        ...
        result_dict["effectiveness_hierarchical"].append(
            effectiveness_scores_hierarchical
        )
        ...
        result_dict["effectiveness_context_dep"].append(
            effectiveness_scores_context_dep
        )
...
def load_accuracies_mu_and_goodman(
    all_paths, n_runs=5, n_epochs=300, val_steps=10, zero_shot=True
):
    """loads all mu and goodman accuracies into a dictionary, val_steps should be set to the same as val_frequency
    during training
    """
    result_dict = {"zs_specific_test_acc": [], "zs_generic_test_acc": []}
