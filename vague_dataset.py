...
class VagueDataSet(torch.utils.data.Dataset):
    """
    ...
    def get_item(
        ...
        sender_target, sender_distractors = self.get_sample(
            concept_idx, context_condition
        )
        receiver_target, receiver_distractors = self.get_sample(
            concept_idx, context_condition
        )
        ...
        receiver_label = [
            idx for idx, obj in enumerate(receiver_input) if obj == receiver_target
        ]
    ...
    def sample_target(self, concept_ranges):
        ...
        """
        sample_target = [
            self.sample_attribute(r[0], r[1]).item() for r in concept_ranges
        ]
        return sample_target

    def sample_distractor(self, concept_ranges, context="fine", overlap=0.1):
        """
        This function generates a distractor object based on the context condition and desired overlap.
        A higher overlap value means the distractor's attributes will be very close to or slightly within
        the target concept's range, making it harder for the model to distinguish between the target and distractors.
        """
        distractor = []
    ...
    def get_sample(self, concept_idx, context_condition):
        ...
        for _ in range(num_distractors):
            distractor = self.sample_distractor(
                concept_ranges, context=context_condition
            )
    ...
    def get_all_concepts(self):
        ...
        for obj in all_objects:
            concept_range = [
                (max(0, value - range_width), min(1, value + range_width))
                for value in obj
            ]

            # Find objects that fit into this concept
            concept_objects = [
                o for o in all_objects if self.satisfies(o, concept_range)
            ]
    ...
    def satisfies(object_attributes, concept_ranges):
        ...
        """
        return all(
            r[0] <= attr <= r[1] for attr, r in zip(object_attributes, concept_ranges)
        )
    ...
    def _get_all_possible_objects(properties_dim):
        ...
        all_objects = list(
            itertools.product(*[dim.tolist() for dim in continuous_dims])
        )
